...
만약, 서로 다른 하위 시스템을 구현하는 팀끼리 협업이 제대로 되지 않거나 이로 인해 번역 계층을 구현함에 있어서 어려움을 느낀다면 이에 대한 대안이 있어야 할 것이다. 이 대안에 대한 선택은 필요로 하는 컴포넌트의 설계 품질과 스타일에 달려 있다.

사용하고자하는 컴포넌트나 하위 시스템의 설계 품질이 떨어지고, 모델이 자신이 개발중인 Context의 모델과 동떨어진다면 어쩔수 없이 번역 계층을 개발하고 유지보수해야 하는 책임을 맡아야 한다.

만약 그렇게 품질이 떨어지지 않고 합리적으로 수용할 수 있다고 한다면, 전적으로 독립적인 모델을 포기하고 그 컴포넌트 Context에 모델링된 모델을 100% 따르는 방법이 있다. 이를 통해 Bounded Context 간의 번역에 따른 복잡도를 제거하고 통합 자체를 단순화시킬 수 있다.
...
일반적으로는 각 Bounded Context마다 통합해야 할 각 외부 Context의 구성요소에 대해 번역 계층을 정의할 것이다. 그런데 어느 특정 하위 시스템을 필요로 하는 것이 많다면 좀 더 유연한 접근법이 필요하다.

어떤 하위시스템을 다른 여러 하위시스템과 통합할 경우, 각 하위 시스템에 대해 번역 계층을 구현한다면, 요구사항 변경과 같은 이유로 코드 변경이 발생할 때 번역 계층을 유지보수할 범위가 매우 넓어질 수 있다.

**만약 하위 시스템이 일관성있게 다른 하위 시스템으로 기능을 제공할 수 있다면, 여러 하위 시스템의 공통의 요구사항을 만족하는 일련의 Service로 구현하는 것도 가능할 것이다.** 하위 시스템 접근과 관련된 프로토콜을 일련의 Service로 정의하라. 프로토콜을 공개하여 자신이 개발 중인 시스템과 통합하고자 하는 모든 이들이 해당 프로토콜을 사용할 수 있도록 하라.
