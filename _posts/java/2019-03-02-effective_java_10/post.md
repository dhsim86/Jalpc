...
작업 도중 예외가 발생하더라도 그 객체는 여전히 정상적으로 사용할 수 있는 상태라면 더욱 멋질 것이다. 특히 checked 예외일 경우 클라이언트가 오류 상태를 복구할 수 있을테니 더욱 유용할 것이다. **호출된 메서드가 실패하더라도 그 객체는 메서드 호출 전 상태로 유지하는 것이 좋다.** 이를 **실패 원자적(failure-atomic)**이라고 한다.
